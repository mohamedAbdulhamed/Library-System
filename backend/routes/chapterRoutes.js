...
router.get('/get-chapters-by-book/:id', (req, res) => {
  const sql = 'SELECT * FROM chapters WHERE book_id = ?';
  const values = [req.params.id];
  db.query(sql, [values] , (err, data) => {
    if (err) return res.status(400).json({err: "Error Occured!", devErr: err});
    if (data.length > 0) return res.status(200).json(data);
    else return res.status(400).json({err: "No Record Found!", devErr: err});
  });
});
