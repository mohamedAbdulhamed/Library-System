...
router.post('/add-book', uploads.single("pdf"),(req, res) => {
  // const { name, description, author, field, dop } = req.body;
  // console.log(name, description, author, field, dop); // defined !!! :)
...
router.post('/update-book/:id', uploads.single("pdf"), (req, res) => {
  const { name, description, author, field, dop } = req.body;
  console.log(name, description, author, field, dop); // undefined ???
  db.query('SELECT * FROM books WHERE id = ?', [req.params.id] , (err, data) => {
    ...
    if (data.length > 0) {
      ...
      if (req.file) {
        // user wants to update the file
        fs.unlink('./public/uploads/' + oldFile, (err) => {
          if (err) return res.status(500).json({err: "Error Occured!", devErr: err});
        });
        // Check if file type is allowed
        const fileExe = req.file.fieldname;
        if (!exe.includes(fileExe)) return res.status(400).json({ err: 'File Type Not Allowed!' });
        // Check if file size is less than 150MB
        if (req.file.size > FILE_MAX_SIZE ) return res.status(400).json({ err: 'File Size Too Large! Max File Size is 150MB' });
        // Get the file name
        fileName = req.file.filename;
      }else {
        // user does not want to update the file
        fileName = oldFile;
      }
      if (req.body.name == '' || req.body.description == '' || req.body.author == '' || req.body.dop == '') {
