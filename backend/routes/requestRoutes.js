...
router.get('/get-request/:id', (req, res) => {
  ...
  db.query(sql, req.params.id , (err, data) => {
    ...
    else return res.status(404).json({err: "Cannot Find Request!", devErr: "Cannot Find Request!"});
  });
});
...
router.get('/get-user-requests/:id', (req, res) => {
  ...
  db.query(sql, id, (err, data) => {
    if (err) return res.status(500).json({err: "Error Occured!", devErr: err});
    if (data.length > 0) return res.status(200).json(data);
    else return res.status(404).json({err: "User Has No Requests!", devErr: "User Has No Requests!"});
  });
});
