...
router.get('/verify', (req, res) => {
  ...
  if (token == null) return res.status(404).json({msg: "Token not found!", loggedIn: false, devErr: "Token not found!"});
  jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (err, user) => {
    if (err) return res.status(500).json({err: "Verifing problem, try again later! ", devErr: err});
...
router.get('/logout', (req, res) => {
  const token = req.cookies.token;
  if (token == null) return res.status(404).json({msg: "Token not found!"});
  jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (err, user) => {
