...
router.get('/verify', (req, res) => {
  ...
  if (token == null) return res.json({msg: "Token not found!", loggedIn: false});
      ...
      if (result.length > 0) {
        bycrypt.compare(req.body.password, result[0].password, (err, response) => {
          ...
          if (response) {
            ...
            return res.json({msg: 'Incorrect Username and/or Password!'});
          }
        });
      } else {
...
router.post('/register', (req, res) => {
  ...
  bycrypt.hash(req.body.password, SALT_ROUNDS, (err, hash) => {
    if (err) return res.json({msg: "Error occured while registering user! 456", err: err});
...
router.get('/logout', (req, res) => {
  const token = req.cookies.token;
  if (token == null) return res.json({msg: "Token not found!"});
  jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (err, user) => {
    if (err) return res.json({msg: "Error: " + err});
